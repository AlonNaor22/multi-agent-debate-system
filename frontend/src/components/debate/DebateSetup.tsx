...
export function DebateSetup({ onStart, isLoading, onViewHistory }: DebateSetupProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!topic.trim()) {
      setLocalError(strings.setup.topicRequired);
      return;
    }
    setLocalError(null);
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          ...
          <input
            ...
            placeholder={strings.setup.topicPlaceholder}
          ...
          >
            {isLoading ? (
              <span className="flex items-center gap-2">
                ...
                {strings.setup.startingDebate}
              </span>
            ) : (
              strings.setup.startDebate
            )}
          </button>
        </div>
      </form>
...
function StyleCard({ style, isSelected, onClick, disabled, colorClass }: StyleCardProps) {
  ...
  return (
    <button
      ...
      `}
    >
      <div className="text-2xl mb-1" aria-hidden="true">{styleIcons[style.name] || '🎭'}</div>
